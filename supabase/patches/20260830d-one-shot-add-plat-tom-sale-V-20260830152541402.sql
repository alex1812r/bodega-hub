-- =============================================================================
-- 20260830d — One-shot: +1 unidad plat-tom-80gr en venta
-- V-20260830152541402 (4a5bef90-fc4a-422e-ab76-e6a0f160d9fd)
--
-- Inserta (o actualiza si existe) linea Platanitos Tom 80Gr ×1; recalcula
-- totales; sube pago_movil + account_in + vault sale_in (cuenta) + balance_ves;
-- stock_movement venta −1. NO toca Polar Light ni compras.
--
-- Idempotente: marker FIX_ADD_PLAT_TOM:V-20260830152541402 en stock_movements.reason
-- =============================================================================

do $$
declare
  v_sale_id uuid := '4a5bef90-fc4a-422e-ab76-e6a0f160d9fd';
  v_product_id uuid := 'f461c9b6-f4ab-415b-9c41-254a15aa0f27';
  ...
  v_marker text := 'FIX_ADD_PLAT_TOM:V-20260830152541402';
...
begin
  ...
  where id = v_product_id
    and sku = 'plat-tom-80gr'
  for update;

  if not found then
    raise exception 'Producto plat-tom-80gr no encontrado';
  end if;

  if v_product.current_stock < v_delta_qty then
    raise exception 'Stock insuficiente plat-tom-80gr: % (necesita >= %)',
  ...
  v_unit_price_ref := v_product.sale_price_ref;
  v_unit_cost_ref := v_product.current_cost_ref;
  ...
  if found then
    ...
    v_unit_cost_ref := v_item.unit_cost_ref_snapshot;
  end if;
  ...
  if v_item_existed then
    update public.sale_items
    set quantity = v_new_qty,
        unit_price_ref = v_unit_price_ref,
        unit_cost_ref_snapshot = v_unit_cost_ref,
        subtotal_ves = v_new_line_subtotal_ves
    where id = v_item_id;
  else
    insert into public.sale_items (
      sale_id, product_id, quantity, unit_price_ref,
      unit_cost_ref_snapshot, subtotal_ves
    ) values (
      v_sale_id, v_product_id, v_new_qty, v_unit_price_ref,
      v_unit_cost_ref, v_new_line_subtotal_ves
    )
  ...
  insert into public.stock_movements (
    ...
    v_sale_id,
    null,
    v_marker || ' Venta ' || v_sale.invoice_number || ' +1 plat-tom-80gr',
    v_sale.user_id,
    v_sale.store_id
  );
  ...
  raise notice
    'OK %: plat-tom qty %→%, total_ref %→%, total_ves %→%, pago_ves %→%, stock %→%',
