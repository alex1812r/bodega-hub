-- =============================================================================
-- 20260815b — One-shot: deposito efectivo + pago + purchase_out baul
-- Compra C-20260815204121850 (Delilicor) — recibida, paid=0.
-- Idempotente: BACKFILL_VAULT:C-20260815204121850
-- =============================================================================
...
declare
  ...
  v_payment_id uuid;
  v_marker text := 'BACKFILL_VAULT:C-20260815204121850';
...
begin
  ...
  where id = '98d5f33c-6343-484e-871e-3c4e6ce99eb5'
  for update;

  if not found then
    raise exception 'Compra C-20260815204121850 no encontrada';
  end if;
  ...
  if exists (
    ...
    where purchase_id = v_purchase.id and status = 'activo'
  ) then
  ...
  insert into public.payments (
    store_id, direction, purchase_id, contact_id, method, currency,
    amount, amount_ves, amount_ref, ref_rate_ves, notes, created_by, created_at, status
  ) values (
    v_purchase.store_id, 'salida', v_purchase.id, v_purchase.supplier_id,
    'efectivo_ves', 'VES', v_amount_ves, v_amount_ves, v_amount_ref,
    v_purchase.ref_rate_ves,
    v_marker || ' pago efectivo historico factura 00016223',
    v_purchase.user_id, v_pay_at, 'activo'
  ) returning id into v_payment_id;

  insert into public.vault_movements (
    store_id, vault_id, type, bucket, amount_ves, amount_ref, payment_id, notes, created_by, created_at
  ) values (
    v_purchase.store_id, v_vault.id, 'purchase_out', 'efectivo',
    v_amount_ves, v_amount_ref, v_payment_id,
    v_marker || ' egreso compra ' || v_purchase.purchase_number,
    v_purchase.user_id, v_pay_at
  );
  ...
  raise notice 'Backfill OK C-20260815204121850: % REF / % VES (payment %)',
