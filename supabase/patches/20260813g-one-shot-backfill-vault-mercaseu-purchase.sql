-- =============================================================================
-- 20260813g — One-shot: deposito efectivo + pago + purchase_out baul
-- Compra C-20260814011924874 (Mercaseu 2030 C.A) — recibida, paid=0.
-- Igual patron que compra informal: deposito efectivo → pago efectivo_ves → egreso.
-- Net baul efectivo = 0.
-- Idempotente: BACKFILL_VAULT:C-20260814011924874
-- =============================================================================
...
declare
  ...
  v_payment_id uuid;
  v_marker text := 'BACKFILL_VAULT:C-20260814011924874';
...
begin
  ...
  where id = 'ef65f6b5-8f92-4477-9315-26b579544492'
  for update;

  if not found then
    raise exception 'Compra C-20260814011924874 no encontrada';
  end if;
  ...
  if exists (
    ...
    raise exception
      'La compra ya tiene pago activo; no crear backfill duplicado.';
  end if;
  ...
  -- 1) Deposito en efectivo (cubre el egreso)
  ...
  insert into public.payments (
    ...
    v_marker || ' pago efectivo historico factura 00005577',
    v_purchase.user_id,
    v_pay_at,
    'activo'
  ) returning id into v_payment_id;

  -- 3) Egreso compra desde efectivo
  insert into public.vault_movements (
  ...
  raise notice
    'Backfill OK C-20260814011924874: deposit efectivo + payment + purchase_out % REF / % VES (payment %)',
