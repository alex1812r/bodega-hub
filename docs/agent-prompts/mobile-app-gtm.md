...
> 2. **Confirma que `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` están en `.env` o `.env.local`** (Next carga ambos; hoy están en `.env`, gitignored). Es lo único que el modo mock no puede validar: login real, refresh de token y Bearer contra RLS. Sin ellas el plan avanza, pero el reporte final dirá "auth real no verificada".
...
6. **Preguntas permitidas (una vez cada una, sin bloquear):**
   - Si ni `.env` ni `.env.local` tienen `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY` válidos y el BFF no arranca en modo supabase: *"Sin credenciales de Supabase en .env/.env.local; desarrollo y pruebo contra el BFF en modo mock (API_DATA_SOURCE=mock, ALLOW_DEMO_AUTH=true). Para probar login real, agrégalas."* y sigues en mock. Vuelves a comprobar ambos archivos al inicio de cada ciclo 9; si aparecen, ejecutas el flujo `00-login` y el test de Bearer contra Supabase real.
...
| Nómina | — | Pendiente móvil | Módulo web nuevo (`/payroll`, ver [`nomina-gtm.md`](nomina-gtm.md) y `modules-catalog.md` §Nómina). No entra en este plan: el cálculo y el pago son tareas de escritorio del dueño. Lo primero a portar cuando se retome es `/payroll/mine` para el cajero (sus recibos y su comisión acumulada), que sí es de uso móvil |
...
5. Arranca el BFF en segundo plano. Para E2E deterministas usa **siempre** mock: `API_DATA_SOURCE=mock ALLOW_DEMO_AUTH=true npm run dev` (estas variables de línea de comandos ganan sobre `.env`, que hoy fija `API_DATA_SOURCE=supabase` y `ALLOW_DEMO_AUTH=false`; Next no pisa variables ya presentes en el proceso). Confirma `GET http://localhost:3000/api/auth/me` con `x-demo-role: admin`: debe devolver el perfil demo; un 401 significa que ganó `.env` y sigues en modo supabase. Para el flujo `00-login` real y la prueba de Bearer (Fase 1.3), levanta una segunda instancia con los valores de `.env` (modo supabase, puerto 3001) solo cuando la necesites.
