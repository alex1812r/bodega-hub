...
Codigo **puro** compartido entre la web (`src/`) y BodegaHub Mobile (`mobile/`).
...
- Solo logica de negocio y datos: permisos, moneda, fechas de Caracas, bancos y
  telefonos venezolanos, metodos de pago, generacion de SKU.
- Se consume **como fuente TypeScript**, sin paso de build. Por eso la web declara
  ...
  `watchFolders` en `mobile/metro.config.js`.
