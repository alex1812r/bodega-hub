...
export async function POST(request: Request) { try { const auth = await requireStorePermission(request, "vault.manage"); return jsonData(await service().deposit(schema.parse(await request.json()), auth.storeId)); } catch (error) { return toErrorResponse(error); } }
