...
/** Confirm upload: verifies Storage object, sets image_url from getPublicUrl. */
export async function POST(request: Request, context: RouteContext<"/api/products/[id]/image">) {
  try {
    ...
    return jsonData(await service.confirmProductImageUpload(id, format, auth.storeId));
  } catch (error) {
...
export async function DELETE(request: Request, context: RouteContext<"/api/products/[id]/image">) {
  try {
    ...
    const { id } = await context.params;
    const service = getProductImagesService();
    return jsonData(await service.deleteProductImage(id, auth.storeId));
  } catch (error) {
    return toErrorResponse(error);
  }
}
