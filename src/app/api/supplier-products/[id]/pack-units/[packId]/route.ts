...
export async function PATCH(request: Request, context: PackUnitRouteContext) {
  try {
    ...
    return jsonData(
      await getSupplierProductsService().updateSupplierProductPackUnit(id, packId, input),
    );
  } catch (error) {
...
export async function DELETE(request: Request, context: PackUnitRouteContext) {
  try {
    ...
    return jsonData(
      await getSupplierProductsService().deactivateSupplierProductPackUnit(id, packId),
    );
  } catch (error) {
    return toErrorResponse(error);
  }
